...
func (s *Service) writeBundleExtras(ctx context.Context, opts BundleOptions, outDir string, priorDotEnv []byte, priorExisted bool) error {
	...
	// Resolve the effective deployment profile for VIBEWARDEN_PROFILE in .env.
	// The base opts.Config.Profile is the base-only value (typically "dev");
	// the production override may set "prod". Mirror the pattern used by
	// readProdTLSDomain / readProdDeployHost in the CLI layer: read the field
	// directly from the production override YAML so we do not depend on a
	// merged Config being threaded through the entire call stack.
	profile := ""
	if opts.Config != nil {
		profile = opts.Config.Profile
	}
	if opts.ProdConfigPath != "" {
		if prodProfile := readProdProfile(opts.ProdConfigPath); prodProfile != "" {
			profile = prodProfile
		}
	}

	// sample.env — always overwrite.
	sampleBody := renderSampleEnv(imageTag, profile, templateKeys)
	...
	//                                   but migrate deprecated OPENBAO_DEV_ROOT_TOKEN
	//                                   → OPENBAO_ROOT_TOKEN in-place (#1345).
	...
	if priorExisted && !opts.Overwrite {
		...
		// but first migrate the deprecated OPENBAO_DEV_ROOT_TOKEN key name
		// (#1345) so preserved .env files don't break prod init+unseal.
		migrated := migrateDeprecatedOpenBaoToken(priorDotEnv)
		if err := s.bundleFS.WriteFile(dotEnvPath, migrated, 0o600); err != nil {
		...
		// VIBEWARDEN_PROFILE + template keys). Keys that already exist in
		// the generator output are NOT duplicated. With --overwrite the
		// merge source is always the freshly-written generator file (no
		// user state leaks through).
		existing, readErr := s.bundleFS.ReadFile(dotEnvPath)
		if readErr != nil && !errors.Is(readErr, fs.ErrNotExist) {
			return fmt.Errorf("reading generator %s: %w", fileDotEnv, readErr)
		}
		merged := mergeDotEnv(existing, renderDotEnv(imageTag, profile, templateKeys))
...
// It always emits VIBEWARDEN_PROFILE (when non-empty) and VIBEWARDEN_APP_IMAGE,
// then any keys discovered in the project .env.template with an empty default
// value. Output is stable for a given input — no timestamps, no randomness.
func renderSampleEnv(imageTag, profile string, templateKeys []string) string {
	var b strings.Builder
	b.WriteString("# sample.env — generated by `vibew bundle`.\n")
	b.WriteString("# Copy to .env and fill in real values before deploying.\n")
	b.WriteString("\n")
	// Emit VIBEWARDEN_PROFILE first so seed-secrets picks up prod mode.
	if profile != "" {
		b.WriteString("VIBEWARDEN_PROFILE=")
	...
	sort.Strings(keys)
	for _, k := range keys {
		if k == "VIBEWARDEN_APP_IMAGE" || k == "VIBEWARDEN_PROFILE" {
			// Do not duplicate the managed keys.
			continue
		}
		b.WriteString(k)
		b.WriteString("=\n")
...
func renderDotEnv(imageTag, profile string, templateKeys []string) string {
	return renderSampleEnv(imageTag, profile, templateKeys)
}

// migrateDeprecatedOpenBaoToken replaces the deprecated OPENBAO_DEV_ROOT_TOKEN
// key with OPENBAO_ROOT_TOKEN in a .env byte slice. It is applied to
// preserved .env files during the priorExisted+!Overwrite path so that
// bundles created before v0.19 do not break prod init+unseal (#1345).
// The replacement is line-level; the value is preserved verbatim.
func migrateDeprecatedOpenBaoToken(content []byte) []byte {
	if !strings.Contains(string(content), "OPENBAO_DEV_ROOT_TOKEN=") {
		return content
	}
	result := strings.ReplaceAll(string(content), "OPENBAO_DEV_ROOT_TOKEN=", "OPENBAO_ROOT_TOKEN=")
	return []byte(result)
}

// readProdProfile reads the profile field from a production override YAML
// without full config merging. This mirrors the pattern used by
// readProdTLSDomain and readProdDeployHost in bundle.go — targeted field
// reads avoid the cost and complexity of merging when only a single value is
// needed. Returns "" when the file is absent, unreadable, or has no profile.
func readProdProfile(prodConfigPath string) string {
	if prodConfigPath == "" {
		return ""
	}
	data, err := os.ReadFile(prodConfigPath) //nolint:gosec // prodConfigPath is the resolved production config path
	if err != nil {
		return ""
	}
	var tree struct {
