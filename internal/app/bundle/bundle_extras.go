...
func (s *Service) writeBundleExtras(ctx context.Context, opts BundleOptions, outDir string, priorDotEnv []byte, priorExisted bool) error {
	...
	// deploy.sh — always overwrite, executable mode. healthPort is baked
	// into the script literal at render time from the merged config; the
	// script itself does not parse YAML. Default 8443 matches the config
	// default when Server.Port is zero/unset (see ADR-088).
	healthPort := 8443
...
//
// The script runs LOCALLY (on the operator's workstation) and ships the
// bundle dir to a remote host via scp + ssh + healthcheck in one command.
// The remote-run convention (ssh into the VPS and run the script there)
// was a bug — see ADR-088 for the full rationale.
//
// `loadCmd` is `docker load -i image.tar && ` by default, or
// `docker compose pull && ` when skipImage is true (registry pull mode).
// `healthPort` is the HTTPS port the sidecar listens on, baked in at render
// time from the merged config (ADR-088 §Application service).
//
// Output is deterministic for a given (projectName, skipImage, healthPort).
...
ssh "$USER_HOST" "cd $REMOTE_PATH && %sdocker compose up -d"
sleep 3
if ! ssh "$USER_HOST" "curl -fsSL -m 10 http://localhost:%d/_vibewarden/health" >/dev/null; then
...
`, projectName, loadCmd, healthPort)
}

// renderBundleReadme produces the README.md shipped inside the bundle.
// Output is stable for a given (projectName, skipImage) and fits under 40
// lines. The single-step "Deploy" section matches the local-run convention
// in `renderDeploySH` so there is exactly one incantation on the page
// (ADR-088).
func renderBundleReadme(projectName string, skipImage bool) string {
	loadNote := "loads the image from `image.tar`"
	if skipImage {
		loadNote = "runs `docker compose pull` to fetch the image from the registry"
	}
	...
	return fmt.Sprintf("# %s deploy bundle\n"+
		...
		"## Deploy\n"+
		"\n"+
		"From the directory that holds this README, run one command on your workstation:\n"+
		"\n"+
		"    ./deploy.sh user@host                   # default remote path: ~/vibewarden-bundle\n"+
		"    ./deploy.sh user@host:/srv/%s           # custom remote path\n"+
		"\n"+
		"`deploy.sh` runs locally. It `scp`s this directory to the host, %s, runs `docker compose up -d`, and probes `/_vibewarden/health` on the remote. Non-zero exit on any failure, with the last 50 log lines dumped to stderr for diagnosis. SSH-key setup and host-key acceptance are assumed to already work via your `~/.ssh/config`.\n"+
		"\n"+
		...
		projectName, projectName, projectName, loadNote)
}
