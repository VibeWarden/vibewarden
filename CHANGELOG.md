...
- **docs(#1456): correct three stale documentation details (consolidates #1282, #1287, #1288).** `decisions/README.md`: the 2026-05-03 audit summary said "42 KEEP" and "3 STATUS BANNERS"; the table supports 45 KEEP (69 ADRs audited − 13 demoted − 11 tombstoned) and PR #1261 added 4 banners (ADR-058, ADR-063, ADR-070, ADR-079). The summary now states the audited ADR range so the arithmetic stays verifiable as new ADRs are appended. `decisions/adr-075-*.md`: gains the Historical banner and lineage note the audit called for but never applied: `vibew deploy` was sunset by ADR-086, and the "local resolution, no remote patching" principle now lives in `vibew bundle` (ADR-085). CHANGELOG v0.18.0: the "Migrating from v0.17.0" table and the `deploy.sh` removal note still prescribed `scp -r .vibewarden/bundle/*`, the dotfile-dropping form replaced by the tar pipe in v0.18.4 (#1217); both now show the tar pipe with an annotation, and the placeholders are bracketed per the cross-LLM literal-vs-template rule. No code changes.
