...
- **chore(#1298): close the Dependabot blind spots that let container image pins drift (audit `audit:2026-05-03`).** Three gaps, one cause: Dependabot only saw a fraction of the images this repo pins. (1) The `docker` ecosystem watched `directory: /`, so only the root `Dockerfile`, `Dockerfile.goreleaser` and `docker-compose.yml` were tracked, which is how `examples/demo-app/Dockerfile` and `test/egress/docker-compose.yml` sat on `alpine:3.21` while root had already moved to 3.24 (both are now on 3.24). `.github/dependabot.yml` now lists all eight directories that hold a real Dockerfile or compose file. They are enumerated rather than globbed on purpose: a `/**` glob would also rewrite the generated bundles under `examples/demo-app/.vibewarden/` and the deliberately mismatched Dockerfile fixture under `internal/cli/cmd/testdata/`, breaking tests on a dependency PR. (2) Dependabot cannot parse `internal/config/templates/*.tmpl` as Docker manifests, and never will, so `internal/config/templates/image_pins_test.go` grows from an OpenBao-specific check into the general rule: every image pinned in any embedded template must carry the same tag as that image in the root `docker-compose.yml` (the file Dependabot does parse), or be listed in `unmonitoredTemplateImages` with a reason. A Dependabot PR against the dev compose now fails the suite until the template is bumped with it, and a new template-only image cannot be added silently. (3) Enabling that rule immediately caught five stale pins in the compose template rendered by `vibew init`, all now synced to the tags the dev stack runs: `curlimages/curl` 8.12.1 to 8.19.0, `prom/prometheus` v3.2.1 to v3.11.2, `grafana/loki` and `grafana/promtail` 3.4.3 to 3.6.10, and `grafana/grafana:11.5.2` to `grafana/grafana-oss:13.0.1`. That last one changes the image name as well as the tag, aligning the template with the dev stack (both are Grafana OSS builds, same env vars and provisioning mounts) so the pin is covered going forward. Two template images stay hand-audited because the dev stack has nothing to pin them against: `otel/opentelemetry-collector-contrib` and `jaegertracing/jaeger`. **Regenerating picks up the newer observability images**, which only affects projects that run the `observability` profile.
