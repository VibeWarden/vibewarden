...
- **test(#1305): coverage backfill for the four `internal/app` packages that sat below the CLAUDE.md 80% floor (audit `audit:2026-05-03`).** `app/upgrade` 76.7% → 89.7%, `app/generate` 75.3% → 87.4%, `app/reload` 84.7% → 90.1%, `app/env` 82.9% → 89.0%. The tests target the cold spots the audit named, not line count: `installBinary`/`atomicReplace`/`extractTarGz`/`downloadFile` (the binary-swap path that rewrites the user's `vibew` on disk — its rollback, temp-file-cleanup, permission-retry, corrupt-archive and non-200 branches were all unverified), `resolveProjectRoot`/`WithConfigSourcePath`/`copyVibewardenYAML`/`generateObservability` plus the `Generate` failure-wrapping paths, the `os.Getwd` fallback and both loader error paths of the ADR-102 env resolver, and the reload event-emission helpers driven through a *failing* event sink to prove that a broken log never aborts a reload or leaves the site registry inconsistent. `internal/app/validate` (79.2%, `RunChecks` at 0%) is included so no `internal/app` or `internal/domain` package is left under the floor. The sudo-retry branch of `installBinary` is exercised through a stub `sudo` on `PATH`, so the suite never escalates privileges. No production behaviour changes beyond the `#1305` fix below.
