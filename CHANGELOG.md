...
- **feat(#1478): per-client-IP lockout on failed admin-token attempts (ADR-114).** `AdminAuthMiddleware` runs at plugin priority 30, ahead of the rate limiter at 50, and `/_vibewarden/*` is unconditionally exempt from rate limiting anyway, so nothing throttled repeated `X-Admin-Key` guesses against `/_vibewarden/admin/*` — every wrong token cost the attacker one 401 and cost you one `audit.auth.failure` line. Ten consecutive failures from one client IP within a 1-minute window now lock that IP out for 1 minute: further requests get `429 Too Many Requests` with a `Retry-After` header and the error code `too_many_failed_attempts` (**not** `rate_limit_exceeded` — a lockout is a different condition with a different remedy), and the token is never compared on that path. A successful authentication clears the counter; the cooldown expiring resets it to zero, so there is no permanent lockout. The token is 256 bits compared in constant time, so this is defence in depth plus an audit-log-volume fix, not a critical patch. The policy is a pure domain state machine (`internal/domain/authguard`, `now time.Time` passed in) behind a new `ports.AuthLockoutGuard`, implemented by an in-memory guard capped at 10 000 client IPs with inline eviction — idle entries are swept first, so an attacker cycling source IPs cannot evict an active lockout, and there is no background GC goroutine to leak per handler `Provision`. The guard is **one shared instance** published through `RuntimeServices`: a Caddy config carries up to three `AdminAuthHandler` instances (admin route, config route, catch-all), and per-handler state would have handed an attacker three separate budgets; it is built once per `vibew serve`, so a config hot-reload does not hand out a fresh budget either. Two deliberate carve-outs: the tokenless admin UI (`/_vibewarden/admin/ui/*`) sits above the check, so console assets never feed the counter and are never throttled; and a request whose client IP cannot be resolved skips the lockout entirely and is evaluated against the token as before, because keying on the empty string would let one attacker lock out every unidentifiable client. `X-Forwarded-For` is not honoured, so behind a connection-terminating proxy the counter degrades to a single global one for admin traffic — it throttles harder, never less. No new config keys (threshold, window and cooldown are fixed exported constants) and no new dependencies.

- **test(#1307): the WAF benchmarks now scan something — body, query parameters and headers (audit `audit:2026-05-03`).** Both bench requests were built by `newBenchRequest()`, a bare `GET /api/resource` with no body, no query string and none of the three headers `waf.ScanRequest` inspects (`Cookie`, `Referer`, `User-Agent`). Every published WAF number was therefore scan *setup*: the 8 KB body buffer was allocated, the query map was built, and then zero rules were evaluated against zero locations. Both request shapes now carry two query parameters and a 117-byte browser `User-Agent`, and a new `BenchmarkProxy_WithWAF_WithBody` adds the same 37-byte JSON payload the domain-level `BenchmarkScanRequest_Typical` uses, so the difference between the two benchmarks is exactly the body scan. A companion `BenchmarkProxy_DirectPassthrough_WithBody` is the baseline for it: a `POST` with a body costs more to construct than a bodyless `GET`, and charging that to the WAF would overstate the overhead. Re-measured on an M1 Max with Go 1.27 (the previous numbers came from a Rosetta VM), **WAF overhead is +26.5 µs on the bodyless shape and +33.9 µs with the body**, not the ~1.4 µs the docs claimed. The headline finding is that cost tracks **inspected bytes, not the presence of a body**: the 37-byte payload adds 7.4 µs, about 0.2 µs per byte, and at that rate the long `User-Agent` costs several times more than the JSON body. `docs/performance.md` is re-derived around that (both results tables, the WAF and AllMiddleware observations, the request-shape and limitations sections), and `docs/faq.md`'s "under 3 microseconds per request" is replaced by the same guidance, since a single headline number is what made this wrong in the first place. Absolute cost is still ~0.03 ms against a < 2 ms P50 WAF budget. Benchmarks and docs only — no production code changed.

- **test(#1305): coverage backfill for the four `internal/app` packages that sat below the CLAUDE.md 80% floor (audit `audit:2026-05-03`).** `app/upgrade` 76.7% → 89.7%, `app/generate` 75.3% → 87.4%, `app/reload` 84.7% → 91.6%, `app/env` 82.9% → 89.0%. The tests target the cold spots the audit named, not line count: `installBinary`/`atomicReplace`/`extractTarGz`/`downloadFile` (the binary-swap path that rewrites the user's `vibew` on disk — its rollback, temp-file-cleanup, permission-retry, corrupt-archive and non-200 branches were all unverified), `resolveProjectRoot`/`WithConfigSourcePath`/`copyVibewardenYAML`/`generateObservability` plus the `Generate` failure-wrapping paths, the `os.Getwd` fallback and both loader error paths of the ADR-102 env resolver, and the reload event-emission helpers driven through a *failing* event sink to prove that a broken log never aborts a reload or leaves the site registry inconsistent. The two rollback branches of the multi-site reload service are covered too (both functions now at 100%): `markSiteError` when the config fails to load *and* the site name is not DNS-safe, so nothing is recorded in the registry, and `rollbackSite`'s no-previous arm, where a proxy reload that fails after a previously-unknown site was added must undo the partial add. `internal/app/validate` (79.2%, `RunChecks` at 0%) is included so no `internal/app` or `internal/domain` package is left under the floor. The sudo-retry branch of `installBinary` is exercised through a stub `sudo` on `PATH`, so the suite never escalates privileges. No production behaviour changes beyond the `#1305` fix below.

- **feat(#1258): install the CLI with `npm install -g @vibewarden/cli` (ADR-112).** VibeWarden's audience mostly has Node installed and reaches for npm before a pipe-to-shell, which also offers no version pinning and no registry discoverability. The new zero-dependency package lives at `npm/` in this repo: `postinstall` downloads the release archive for the package's *own* version (the npm version is always the VibeWarden release version, so `@X.Y.Z` pins exactly and bare `latest` follows the newest release), verifies its SHA-256, extracts `vibew` into `vendor/`, and a Node shim at `bin/vibew` spawns it. The installed binary is byte-identical to the GitHub Release artifact. **Checksums are embedded in the published npm tarball at publish time, not fetched at install time** — a checksum fetched from the same host as the artifact only proves transport integrity, while an embedded one is pinned by your lockfile's integrity hash; combined with `npm publish --provenance` this is the most verifiable install path VibeWarden offers. Nothing unverified ever becomes executable: the archive is verified in memory and the binary is atomically renamed into place only after the digest matches. Unsupported platform, network failure, HTTP 404, redirect loop, checksum mismatch and `EACCES` each exit 1 with a distinct, actionable message rather than a generic "install failed". Zero runtime and install-time dependencies — `node:https`/`zlib`/`crypto`/`fs` only, including a hand-rolled tar reader, since the obvious helpers (`tar`, `adm-zip`, `yauzl`) are ISC, outside the approved license set. Publication is a new `npm` job in `release.yml` gated on the release job, idempotent on re-run, with prereleases going to the `next` dist-tag so they can never land on `latest`. The publish step is additionally gated on `NPM_TOKEN` being set, so a `v*` tag pushed before the npm org is claimed skips publishing with a workflow notice instead of failing the whole Release workflow on an unauthenticated `npm publish`. `scripts/install.sh` is unchanged and stays the documented path for machines without Node; both install commands are now emitted in the agent kickoff prompts (`vibew prompt-template`, ADR-101) and a golden test fails if either is dropped. **Native Windows is not covered**: `.goreleaser.yml` builds `linux` and `darwin` only, so no Windows binary has ever been published; `win32` gets an explicit error pointing at WSL2 and the shell installer. **`vibew upgrade` must not be used on an npm install** — it replaces the binary inside the package's `vendor/` directory, desynchronising it from the package version until the next `npm install -g` silently reverts it; documented in `docs/upgrading.md` and the package README, with a follow-up needed to make the binary refuse. Requires Node >= 22; `make check` now runs the package's `node --test` suite.

- **feat(#1306): container resource limits on the generated `vibewarden` sidecar service (ADR-111).** Neither compose template capped the sidecar, so a memory leak or a goroutine explosion in it consumed host RAM/CPU/PIDs until the VPS died — taking down the app the sidecar exists to protect. Three new keys under the existing `server:` block render as `mem_limit`, `cpus` and `pids_limit` on the `vibewarden` service in both `docker-compose.yml.tmpl` (single-app) and `sidecar-compose.yml.tmpl` (multi-app): `server.mem_limit` (default `"512MB"`, accepts `"512M"`/`"1GB"`/a plain byte count), `server.cpu_limit` (default `1.0`, in cores) and `server.pids_limit` (default `200`). Setting one to `0` (or `""` for `mem_limit`) **omits the key entirely** rather than emitting a `0`; negative or malformed values are rejected by `vibew validate` / `vibew bundle` with the field named, e.g. `server.cpu_limit must be >= 0 (0 disables the limit), got -0.5`. The memory cap also emits `GOMEMLIMIT` at 90% of the byte count: without it, capping memory would make the sidecar *more* likely to be OOM-killed, because the Go GC never sees the cgroup ceiling. `GOMAXPROCS` needs no counterpart — Go has read the cgroup CPU quota by default since 1.25. The legacy service-level keys were chosen over `deploy.resources.limits` on support-breadth grounds; both spellings were verified enforced under plain non-swarm `docker compose up` on Docker 29.7.2 / Compose v5.3.1. Enforcement is proven by `test/integration/compose_limits_test.go`, which starts a real container from the rendered file and asserts `HostConfig.Memory=536870912`, `NanoCpus=1000000000`, `PidsLimit=200` via `docker inspect` — a rendered-string assertion would pass just as happily against a key Docker ignores. These three keys are **generate-time only**: the running sidecar never reads them, so changing one requires regenerating and recreating the container, not a restart. No new dependencies.
...
- **BEHAVIOUR CHANGE (#1511): the `auth.ui` block now takes effect on projects that already set it, and `auth.ui.mode: custom` finally stops serving the built-in pages.** `auth.ui.*` was accepted and validated but never reached the auth plugin, so every existing `auth.mode: kratos` project has been running on the adapter's own defaults. Fixing that changes two things you can see at runtime. **Colours:** a project that set `primary_color`/`background_color` gets those colours for the first time, and a project that set neither switches from the adapter fallback `#F3F4F6` to the documented `auth.ui.background_color` default `#1a1a2e`, so the built-in login page goes from light grey to dark. Set `auth.ui.background_color: "#F3F4F6"` to keep the old look. **Custom mode:** a project on `auth.ui.mode: custom` was still being served (and routed) the built-in `/_vibewarden/{login,registration,recovery,verification,settings}` pages as a de facto fallback; it now gets no built-in pages at all, and unauthenticated requests redirect to its own `auth.ui.login_url`, which is what that mode always claimed to do. If that URL is stale or unrouted, login breaks on upgrade, so verify it before regenerating; see the migration note in `docs/upgrading.md`. One more upgrade-time edge: `auth.ui.logo_url` is now validated as a browser asset URL, so a previously inert malformed value (`logo.png`, `javascript:...`, `//host/x`) fails `Config.Validate()` on the startup path and the sidecar refuses to boot rather than warning.

- **BEHAVIOUR CHANGE (#1478): the admin API can now answer `429`, and the tenth failed admin-auth attempt is reported under a new audit event type.** Any client or script that treats "not 200 and not 401" as a transport error against `/_vibewarden/admin/*` sees a new failure mode; the 429 carries `Retry-After` and, deliberately, no `WWW-Authenticate`. The new `audit.auth.lockout` event type is additive, so consumers switching on `event_type` keep working — but the failure that trips the lockout now emits `audit.auth.lockout` **instead of** `audit.auth.failure`, and requests arriving during the cooldown emit no audit event at all, so brute-force detectors that count `audit.auth.failure` will see fewer of them. Collapsing a probing episode into one audit record is the point of the change. `schema/v1/event.json` is untouched: the `audit.*` stream is a separate JSONL wire format that has never been described there (ADR-114 §7). A team sharing one egress IP can lock itself out for a minute by fat-fingering the token ten times; it is self-healing, and the admin console keeps loading.

- **BEHAVIOUR CHANGE (#1306): regenerating now produces a resource-capped sidecar where it previously had none.** Any existing project that re-runs `vibew generate` or `vibew bundle` gets `mem_limit: 536870912`, `cpus: "1"` and `pids_limit: 200` on its `vibewarden` service, plus `GOMEMLIMIT`. That is the point of #1306, but it is a tightening: on a constrained or unusually loaded host the sidecar can now be OOM-killed by a limit VibeWarden chose (with `restart: unless-stopped` that costs seconds of proxy downtime instead of a dead host). Tune `server.mem_limit` / `server.cpu_limit` / `server.pids_limit`, or set them to `0` to restore the previous uncapped behaviour. **Only the `vibewarden` service is affected** — `app`, `kratos`, `postgres`, `redis` and `openbao` stay uncapped — and projects using `overrides.compose_file` get no limits at all, since that file is copied verbatim.

- **chore(#1505): batch the four Dependabot image-pin PRs of 2026-09-04 (supersedes #1499, #1500, #1501, #1502) -- one bump applied, two rejected, one obsolete.** Second batch after #1495 / PR #1498. **Applied:** `grafana/grafana-oss` 13.0.1 -> 13.0.2 (#1502), a patch, bumped at both pin sites (dev compose and `docker-compose.yml.tmpl`) so the drift guard from #1298 stays green. Verified on the real image: `/api/health` reports `"version": "13.0.2"` with `"database": "ok"`, and the exact compose healthcheck command (`wget -q --spider http://localhost:3000/api/health`) still runs inside it, which is not a given after Loki 3.7 went distroless and broke that pattern in #1495. **Rejected: `quay.io/openbao/openbao` 2.5.5 -> 2.6.2 (#1501)**, reaffirming the deliberate 2.5.x hold from #1291 (PR #1469). The rejection is now proven rather than argued: `docker run --entrypoint id` reports `uid=100(openbao)` on 2.6.2 against `uid=0(root)` on 2.5.5, and `seed-secrets` writes the generated root token and unseal key back into the owner-only bind-mounted `.credentials` file (`0600`, host-uid-owned since #1458), so the generated bundle's seeding step fails as a non-root uid. 2.6.0 also deprecates the `file` storage backend the prod profile uses, for removal in 2.7.0. Both reasons need a verified upgrade with a credentials-ownership fix and a storage migration, not a version-sync commit, so the pin stays and a Dependabot `ignore` rule now covers it. That rule lists **`version-update:semver-minor` as well as `semver-major`**, unlike the postgres and redis rules from ADR-113: the break arrives on a minor here, so a major-only rule would have let 2.6.x straight back in. Patch releases within 2.5.x keep flowing, so security patching is unaffected. **Rejected: `maven` `3.9-eclipse-temurin-25-alpine` -> `3-eclipse-temurin-26-alpine` (#1499).** The Spring Boot example's `pom.xml` sets no `<java.version>`, so javac targets whatever JDK the build stage ships; a Temurin 26 build stage against the Temurin 25 runtime (bumped in #1495) emits class-file version 70 and the container dies at start with `UnsupportedClassVersionError`, having built perfectly green. Both stages stay on Java 25, which is the LTS. Dependabot treats the two images as independent and has now proposed the split in both directions (runtime-only in #1494, caught by hand in #1495; build-only here), so the fix is structural rather than another manual catch: the `maven` image is ignored outright on the `docker` entry, leaving the runtime `eclipse-temurin` bump as the single trigger for moving the pair by hand. Ignoring `maven` entirely costs nothing because its `3.9-...` tag is floating: Maven's own patch releases arrive on the next rebuild without a PR. The manual half is closed by a new **`TestSpringBootExample_BuildAndRuntimeShareJavaMajor`**, a fourth pin site with a constraint no tag-equality check can express, since the two images have different names and nothing in `image_pins_test.go` compared them. It was verified to fail against the exact #1499 diff before being committed. The example was rebuilt and run once on the unchanged pins: HTTP 200, `openjdk version "25.0.4" ... Temurin-25.0.4+7 (build 25.0.4+7-LTS)`. **Obsolete: `docker` `27-dind` -> `29-dind` (#1500).** `test/fixtures/deploy-server/` is an orphan. Its only consumer, `test/integration/deploy_multisite_test.go`, was deleted by the `vibew deploy` sunset (#1051, PR #1064, ADR-086); nothing in the repo has referenced the fixture since, so the bump would have maintained a corpse. The fixture is deleted along with its `/test/fixtures/deploy-server` entry in `.github/dependabot.yml`, and the `make integration` comment that still advertised "Docker-in-Docker scenarios" is corrected to what `test/integration/` actually holds. Also fixes a stale cross-reference in the dev compose: the OpenBao pin comment named `TestOpenBaoImagePin_TemplateMatchesDevCompose`, which #1298 generalised into `TestTemplateImagePins_MatchDevCompose`. **No user-visible change beyond Grafana**: regenerating picks up `grafana-oss:13.0.2`, which only affects projects running the `observability` profile.
- **chore(#1495): batch the ten open Dependabot image-pin PRs — eight bumps applied, two majors deliberately rejected (ADR-113, supersedes #1485-#1494).** PR #1484 (#1298) taught Dependabot to see the compose and example-Dockerfile image pins, and it immediately opened ten PRs. **Applied:** `grafana/loki` 3.6.10 → 3.7.7, `prom/prometheus` v3.11.2 → v3.14.0, `curlimages/curl` 8.19.0 → 8.22.0 (dev compose + `docker-compose.yml.tmpl` for all three), `golang` 1.26 → 1.27 in `examples/demo-app` (with `go.mod` `go 1.27`, or our own `RuleToolchainMatch` doctor check fails on our flagship example), `node` 22 → 26 in `examples/nextjs` (3 stages) and `examples/node-express` (2 stages), `python` 3.13 → 3.14 in `examples/python-flask` (2 stages), and `eclipse-temurin` 21 → 25 in `examples/spring-boot` — **both** stages, since Dependabot's #1494 touched only the `-jre-alpine` runtime and leaving `maven:3.9-eclipse-temurin-21-alpine` behind would have hidden a JDK-21-builds-for-JRE-25 drift until someone set `<java.version>`. `grafana/promtail` goes to **3.6.11, not 3.7.7**: Promtail has no 3.7 line — 3.6.11 (2026-05-13) is its last release, since Grafana deprecated it in favour of Alloy. 3.6.11 pushes to Loki 3.7.7 over the stable `/loki/api/v1/push` v1 API, so the divergence is safe but permanent, and it is commented at both pin sites because the drift guard keys on image *name* and cannot catch it. **Rejected:** `postgres` 17 → 18 (#1489) and `redis` 7 → 8 (#1485) stay pinned, with a rationale comment at every pin site and `version-update:semver-major` ignore rules on the `docker-compose` Dependabot entry so they stop reopening weekly; minor and patch updates keep flowing for both. Postgres 18 changes the catalog version, so every existing `kratos-db` volume refuses to start, and PG18's checksum-enabling `initdb` makes even the `pg_upgrade` escape hatch require `--no-data-checksums`; nothing in PG18 is reachable from VibeWarden. The Redis rejection is **not** the license argument the issue assumed: `redis:7-alpine` is 7.4.x, dual RSALv2/SSPLv1, and has been off the approved list since March 2024, while `redis:8-alpine` adds AGPLv3 as a third option — pinning 7 buys no license improvement, and ADR-113 rules that the approved-license list governs linked and redistributed code, not a third-party server image the user's own daemon pulls and VibeWarden talks to over a socket. The real reasons are that our entire Redis surface is `HMGET`/`HMSET`/`EXPIRE` in one `EVAL`'d script plus `PING` (Redis 8 neither breaks nor improves it) and that `redis:8-alpine` bundles the query/JSON/TimeSeries/Bloom modules at 39.0 MB against 16.3 MB — a 2.4x size increase on a token-bucket hash. The destination is Valkey (BSD-3, 21.2 MB), tracked by #1497; the pin and the ignore rule go away together when it lands. **Third pin site closed:** `TestIntegrationTestImagePins_MatchDevCompose` in `internal/config/templates/image_pins_test.go` now walks `internal/` for `*_integration_test.go` and holds every testcontainers image literal to the dev compose tag. It found two Postgres pins already drifted to `postgres:16-alpine` (`internal/adapters/postgres/audit_integration_test.go`, `internal/adapters/kratos/adapter_integration_test.go`), so CI had been certifying a major nobody runs; both are now on 17. **Two things the bump forced that the design did not predict.** (1) `grafana/loki:3.7.x` is **distroless** — the image holds `/usr/bin/loki` and nothing else — so the `/ready` healthcheck in both compose files died with `exec: "/bin/sh": no such file or directory`, the container was marked unhealthy, and every `condition: service_healthy` dependent refused to start. Docker cannot probe a container from outside, so loki's healthcheck is removed and its three dependents (promtail, otel-collector, grafana) move to `condition: service_started`, exactly as `otel-collector` already does for the same reason; all three retry their own pushes and queries. Verified after the change on a regenerated demo-app stack: `/ready` returns 200 once warm and promtail 3.6.11 lines are queryable out of Loki 3.7.7 under the `container` label, so the cross-version push path is proven by data landing, not by a green container. (2) **`examples/node-express` had never built** — on `node:22-alpine` either, so this is not a Node 26 regression. `COPY package.json package-lock.json* ./` copies nothing (the example deliberately ships no lockfile) and the following `npm ci --omit=dev` hard-fails with `EUSAGE`; it is now `npm install --omit=dev --no-audit --no-fund` with a comment. All five bumped examples were then built **and run**, each answering 200 on its health endpoint; the Spring Boot one reports Java 25.0.4 with Tomcat 11.0.24 and needed no `<java.version>` change. **Regenerating picks up the newer Loki, Promtail, Prometheus and curl images**, which only affects projects running the `observability` profile, and the generated compose file now carries the Postgres and Redis pin rationale as comments into the user's own stack.
...
- **chore(#1454): batch the outstanding dependency updates (supersedes Dependabot #1432, #1443, #1444, #1445, #1447, #1450, #1451, #1452, #1453).** Every bump was audited against the exact call sites VibeWarden uses; `comm` of the old and new `go.sum` module paths shows **zero new module paths** in either Go module, so no new licenses required review.
  - **OpenTelemetry group** — `go.opentelemetry.io/otel` and the `metric`/`trace`/`sdk`/`sdk/metric` modules v1.44.0 → v1.46.0, `log`/`sdk/log`/`otlploghttp` v0.20.0 → v0.22.0, `exporters/prometheus` v0.66.0 → v0.68.0, `contrib/bridges/otelslog` v0.19.0 → v0.20.1. Our surface (`internal/adapters/otel/`, `internal/middleware/`, `internal/adapters/log/`) is the stable `otel`, `metric`, `trace`, `propagation`, `attribute`, `codes`, `sdk/{trace,metric,log,resource}` and OTLP/Prometheus exporter constructors — all unchanged across these releases. The same pass pulls the remaining OTel indirects into lockstep (`contrib` instrumentation/propagators/exporters → v0.71.0 / v1.46.0, `otel/exporters/stdout*` and the gRPC OTLP variants → v1.46.0 / v0.22.0, `proto/otlp` v1.11.0), which closes the version skew tracked in #1297.
  - **`github.com/redis/go-redis/v9`** v9.21.0 → v9.22.0. Our surface is `redis.NewScript` + `Script.Run(...).Int64Slice()` (`internal/adapters/ratelimit/redis_adapter.go`) and `redis.Cmdable`/`ParseURL`/`Options`/`NewClient` (`internal/plugins/ratelimit/plugin.go`); none of it changed.
  - **`github.com/santhosh-tekuri/jsonschema/v6`** v6.0.2 → v6.0.3 (patch). Used only in `internal/domain/llm/response_validator.go`.
  - **testcontainers group** — `testcontainers-go` and the `postgres`/`redis` modules v0.43.0 → v0.44.0. These are integration-test-only (`//go:build integration`); `go vet -tags integration ./...` is clean across the whole repo.
  - **Docker base image and Go toolchain** — `golang:1.26.6-alpine` → `golang:1.27.1-alpine`. Go 1.27 is the current stable release, and our own `RuleToolchainMatch` doctor check compares the builder-stage tag's major.minor against the `go` directive, so the base-image bump is carried through to `go 1.27` / `toolchain go1.27.1` in `go.mod` and `go-version: "1.27.1"` in `ci.yml`, `release.yml`, and `release-dryrun.yml`, keeping the toolchain single-sourced as in the go1.26.6 bump. The pinned `golangci-lint` in `ci.yml` goes to v2.13.2 with it: v2.11.4 is built with Go 1.26 and exits 3 on the `go 1.27` directive, which kept the required `Lint` check red. The documented Go prerequisite follows too — the README badge and the `CONTRIBUTING.md` prerequisite now read 1.27.
  - **Examples** — demo-app `modernc.org/sqlite` v1.55.0 → v1.57.0 (used only through `database/sql`, no direct API surface); Next.js example `next` 16.3.0 → 16.3.4 (`npm audit` also resolves `nanoid` to 3.3.18, clearing GHSA-2v37-7h3g-55p8; `npm run build` verified); node-express example `express` ^4.21.2 → ^5.2.1; spring-boot example `spring-boot-starter-parent` 3.5.5 → 4.1.1.
  - The two example majors were verified by building and running them, not by skimming release notes. **Express 5** removes `res.send(status)`, `res.json(obj, status)`, `app.del`, `req.param()`, `res.sendfile` and changes path-pattern matching; the example uses none of them (literal routes, `res.json`, `req.headers`, `app.listen`) and all three endpoints return identical payloads under 5.2.1. Node ≥ 18 is required and the Dockerfile already pins `node:22-alpine`. **Spring Boot 4.1.1** moves to Spring Framework 7 and Tomcat 11 (Jakarta Servlet 6.1); the Java baseline stays 17 and the Dockerfile already builds and runs on Temurin 21. This required **dropping the `<tomcat.version>10.1.52</tomcat.version>` override** added in #650 for a Trivy finding: under Boot 4 that pin would *downgrade* the managed Tomcat 11.0.24 to a 10.1.x line that no longer matches the Jakarta EE baseline. The packaged jar was confirmed to bundle `tomcat-embed-core-11.0.24` and `spring-web-7.0.9`, and all four endpoints (`/health`, `/`, `/public`, `/protected`) were smoke-tested against the running jar. Neither major needed source changes.
  - `github.com/fsnotify/fsnotify` (#1294) is already at the latest release, v1.10.1 — no change needed.

- **chore(#1298): close the Dependabot blind spots that let container image pins drift (audit `audit:2026-05-03`).** Three gaps, one cause: Dependabot only saw a fraction of the images this repo pins. (1) The `docker` ecosystem watched `directory: /`, and it never fetches compose files at all (dependabot-core keeps only Kubernetes/Helm YAML), so the only tracked manifests were the root `Dockerfile` and `Dockerfile.goreleaser`. That is how `examples/demo-app/Dockerfile` and `test/egress/docker-compose.yml` sat on `alpine:3.21` while root had already moved to 3.24 (both are now on 3.24). `.github/dependabot.yml` now lists the seven directories that hold a Dockerfile under the `docker` ecosystem, plus a separate `docker-compose` ecosystem entry for the two that hold a compose file (`/` and `/test/egress`). `test/egress` holds no Dockerfile, so listing it under `docker` would only have produced a weekly `No Dockerfiles nor Kubernetes YAML found` job error while leaving its alpine pin unwatched. Directories are enumerated rather than globbed on purpose: a `/**` glob would also rewrite the generated bundles under `examples/demo-app/.vibewarden/` and the deliberately mismatched Dockerfile fixture under `internal/cli/cmd/testdata/`, breaking tests on a dependency PR. (2) Dependabot cannot parse `internal/config/templates/*.tmpl` as Docker manifests, and never will, so `internal/config/templates/image_pins_test.go` grows from an OpenBao-specific check into the general rule: every image pinned in any embedded template must carry the same tag as that image in the root `docker-compose.yml` (the file the new `docker-compose` entry makes Dependabot parse), or be listed in `unmonitoredTemplateImages` with a reason. A Dependabot PR against the dev compose now fails the suite until the template is bumped with it, and a new template-only image cannot be added silently. (3) Enabling that rule immediately caught five stale pins in the compose template rendered by `vibew init`, all now synced to the tags the dev stack runs: `curlimages/curl` 8.12.1 to 8.19.0, `prom/prometheus` v3.2.1 to v3.11.2, `grafana/loki` and `grafana/promtail` 3.4.3 to 3.6.10, and `grafana/grafana:11.5.2` to `grafana/grafana-oss:13.0.1`. That last one changes the image name as well as the tag, aligning the template with the dev stack (both are Grafana OSS builds, same env vars and provisioning mounts) so the pin is covered going forward. Two template images stay hand-audited because the dev stack has nothing to pin them against: `otel/opentelemetry-collector-contrib` and `jaegertracing/jaeger`. **Regenerating picks up the newer observability images**, which only affects projects that run the `observability` profile.

- **chore(#1291): bump the OpenBao image in the `vibew init` compose template from 2.2.0 to 2.5.5 and add a drift regression test.** The template is what every new deployment runs, and it had been left behind when the dev `docker-compose.yml` moved to 2.5.2, so users exercised a version nobody tested. Both OpenBao services in the template (`openbao` and `seed-secrets`) and the dev compose file now pin 2.5.5, and `internal/config/templates/image_pins_test.go` fails if the two files disagree or if the two template services disagree with each other. The pin deliberately stays on the 2.5.x line rather than the current 2.6.2: from 2.6.0 the `quay.io/openbao/openbao` image runs as the non-root `openbao` user, and `seed-secrets` writes the generated root token and unseal key back into the owner-only bind-mounted `.credentials` file (`0600`, since #1458), which that change would break. 2.6.0 also deprecates the `file` storage backend used by the prod profile for removal in 2.7.0, so the 2.6.x move needs its own verified upgrade rather than a version-sync commit.

- **chore(#1405): extract the symlink-containment check shared by the two bundle walkers into `containsPath` (`internal/app/bundle/containment.go`).** The separator-terminated check (`resolved == absRoot || strings.HasPrefix(resolved, absRoot+string(os.PathSeparator))`) was inlined in both `input_digest.go` (#1223) and `staleness.go` (#1274), and the drift between those two copies is exactly what #1274 had to fix. Both walkers now call the one helper, which is unit-tested against the sibling-prefix case the bare-`HasPrefix` bug turned on (`/proj-secret` must not be admitted when the root is `/proj`). Pure refactor: no behaviour change, no new dependencies.

- **docs(#1312): correct the stale performance-target comment on `BenchmarkScanRequest_Typical` (audit `audit:2026-05-03`).** The comment claimed a flat "< 1 ms per operation" target, which reads as a fixed per-request cost the benchmark is expected to hold. `waf.ScanRequest` evaluates the whole ruleset once per inspected value, so the cost tracks inspected bytes and a body approaching the 8 KB `maxBodyBytes` scan cap lands in the low milliseconds, past the number the comment named. The comment now states the measured baseline for the benchmark's 37-byte JSON body (~24 µs/op, 18 allocs/op on darwin/arm64, Apple M1 Max, Go 1.27), says the cost scales with inspected bytes up to the cap, and points at `docs/performance.md` for the published proxy-level figures rather than restating them. The 381 µs/op in the audit finding came from a Rosetta VM, the same measurement environment corrected in #1307. Comment only: no test or production behaviour changed.

### Fixed

- **fix(#1540): `security_headers` now covers VibeWarden's own routes, not just proxied app responses.** The handler was appended to the catch-all app route only, so every sidecar-owned route answered with no security headers at all: the built-in login page (`/_vibewarden/login`), the actual credential form, had no `X-Frame-Options` and no CSP and could be framed by another origin, the admin API (`/_vibewarden/admin/*`) returned JSON without `nosniff` on both `200` and `401`, and the Kratos surface (`/self-service/*`, `/_vibewarden/config`) was equally bare. The layout dates from #981 and was found by black-box testing of v0.22.0-rc. The handler is now emitted as a global, unmatched, non-terminal first route ahead of every plugin route, so one configured set applies to every response the sidecar serves, including `/_vibewarden/health` and the metrics route. Operator `response_headers` rules still override security headers on the app-proxied route: the global route runs before the catch-all chain and both apply their ops to the same response header map. The `security-headers` plugin no longer contributes a handler of its own — its copy landed in `ExtraHandlers`, which the catch-all chain inserts *after* the `response_headers` handler, so it ran last and silently clobbered operator rules (a real Caddy returned `DENY` from `frame_option` where `response_headers` asked for `SAMEORIGIN`); the plugin is now config validation and lifecycle only, and the override is pinned by a test that drives a real Caddy with the plugin wired in exactly as the serve path wires it. `response_headers` is applied to the app-proxied route only, never to VibeWarden's own routes, so it cannot override a security header on the login UI, the admin API or the Kratos proxy; `docs/configuration.md`, `llms-full.txt` and `vibewarden.reference.yaml` now say so. HSTS is still emitted only when `tls.enabled: true`. No config change is needed: enabling `security_headers` was always supposed to mean this. The regression test drives a real Caddy instance built from the production config and asserts the header set on `/_vibewarden/login`, the `401` from `/_vibewarden/admin/users`, and `/self-service/login/browser` matches the set on `/`; a config-shape assertion could not have caught this, since the handler JSON was correct and simply never in the chain for those paths. Removing the plugin's handler also removed the only place that resolved the structured `security_headers.csp.*` block into a header value for `vibew eject` and multi-site `serve`: both builders read the raw `content_security_policy` string, so a `csp:`-only config would have emitted no `Content-Security-Policy` at all on those two paths (single-site `serve` escaped it only because it carried a duplicate resolver). CSP resolution now lives in one place, `config.SecurityHeadersConfig.ResolvedCSP()`, called by all three `ProxyConfig` builders with both private copies deleted, and is pinned by tests asserting the header value in the generated eject and multi-site Caddy JSON rather than in the intermediate `ProxyConfig`. `securityheaders.Config` is trimmed to the three fields the plugin still reads, so nothing can mistake it for a live source of header values. The `Secure` attribute missing from the Kratos CSRF cookie, reported in the same issue, is not addressed here.

- **fix(#1536): `vibew add auth` and `vibew wrap --auth` wrote Kratos URLs the sidecar container cannot reach, so the documented auth path was broken on first run.** Both write paths (the `add auth` YAML block and the `wrap` scaffold template) pointed `kratos.public_url` / `kratos.admin_url` at `http://localhost:4433` / `:4434`, but the sidecar dials those from inside its own container, where `localhost` is the sidecar and not the `kratos` service: the admin API answered `503 service_unavailable` (`dial tcp [::1]:4434: connect: connection refused`), `/self-service/login/browser` was a 502, and unauthenticated requests were redirected off the sidecar to `http://localhost:4433/...` because the same block wrote an absolute `auth.login_url` to a port the generated stack never publishes. Both commands now write `http://kratos:4433` / `http://kratos:4434` (matching `examples/demo-app/vibewarden.yaml`, and already understood by `vibew status`, which rewrites that host to a published port when probing from the host) and omit `auth.login_url` entirely, so the middleware's sidecar-relative `/self-service/login/browser` default applies and the login flow stays on the sidecar. **Existing projects are not migrated**: a `vibewarden.yaml` already generated by either command keeps the broken values, so fix it by hand (set both URLs to the `kratos` host and delete `auth.login_url`) or delete the two blocks and re-run the command. Run the binary outside Docker and the `127.0.0.1` defaults still apply; set the URLs to whatever resolves for you. The false claim in `vibewarden.reference.yaml` that the generated compose injects `VIBEWARDEN_KRATOS_PUBLIC_URL` / `VIBEWARDEN_KRATOS_ADMIN_URL` is corrected — no such variable is emitted, so the config file is the only source of these URLs (setting them yourself in a hand-written compose file still overrides, like any key). `TestGenerate_Integration_AddAuthKratosURLsAreComposeServiceNames` and `TestNewWrapCmd_AuthKratosURLsAreComposeServiceNames` pin the invariant end to end, one per entry point: each runs the real command, loads the resulting config, generates the stack, and requires every Kratos URL host to be a service defined in that same `docker-compose.yml`. An assertion on the literal string would have been just as green before the fix.

- **fix(#1512): the built-in login page no longer links to self-service flows that are switched off.** With `selfservice.flows.registration.enabled: false` (or `recovery.enabled: false`) in Kratos, the login page still rendered "Register" and "Forgot password?", and following either landed on a flow Kratos refuses to create. The fix is one setting per flow rather than a runtime query: the new `auth.ui.show_registration` and `auth.ui.show_recovery` (both default `true`) each drive three things at once, so the page and Kratos cannot disagree. Set to `false` and the link disappears from the login page, the built-in page itself (`/_vibewarden/registration`, `/_vibewarden/recovery`) returns 404 instead of a form that cannot submit, and the generated `kratos/kratos.yml` gets `selfservice.flows.<flow>.enabled: false`. `show_registration: false` is the closed-registration deployment where an admin creates every user. Both keys are `*bool` internally, so unset means enabled and existing projects see no change; setting either to `false` requires regenerating (`vibew generate` / `vibew bundle`) for the Kratos half to take effect. The generated-artifact half is pinned by a test that parses the rendered `kratos.yml` rather than asserting on the template string, and the config-to-HTML half by a test that starts the real plugin from a `config.Config` and greps the served page, the same end-to-end shape #1511 needed. No new config keys outside `auth.ui` and no new dependencies.

- **fix(#1511): `auth.ui` branding is actually rendered — the whole block was accepted, validated, and then dropped on the floor.** `RegisterBuiltinPlugins` never populated the auth plugin's `UI` field, so *no* `auth.ui.*` key reached the built-in pages: `app_name` and `logo_url` were never rendered (the reported bug), `primary_color` and `background_color` silently fell back to the adapter's own defaults, and `auth.ui.mode: custom` did not stop the sidecar serving and routing its own `/_vibewarden/{login,registration,recovery,verification,settings}` pages. The config → registry → plugin → page chain is wired end to end and pinned by `internal/plugins/builtin_authui_test.go`, which starts the real plugin from a `config.Config` and greps the HTML the internal UI server returns — the previous tests all constructed `authui.AuthUIConfig` by hand, which is why five dead keys stayed green. Branding now renders on all five pages: `app_name` prefixes the `<title>` and appears above the form, `logo_url` renders as an image above it. Two new keys fold in #1525: `auth.ui.favicon_url` (linked as the page icon) and `auth.ui.custom_css_url` (a stylesheet linked *after* the built-in `<style>`, so operator rules win — the escape hatch for anything the colour knobs cannot express). All three asset keys must be an absolute `http(s)` URL or a root-relative path; `javascript:`, `data:` and protocol-relative `//host` values are rejected by `vibew validate` with the key named, on top of the `html/template` escaping that already covered injection. Assets served by your own app need an `auth.public_paths` entry, or the login page cannot load them while the visitor is unauthenticated. **Two visible changes for existing projects on `auth.mode: kratos`:** a project that set `primary_color`/`background_color` gets those colours for the first time (and one that set neither now picks up the `auth.ui.background_color` default `#1a1a2e` in place of the adapter's `#F3F4F6`), and a project on `auth.ui.mode: custom` stops being served the built-in pages and is redirected to its own `login_url`, which is what that mode always claimed to do. No new dependencies.

- **fix(#1513): the admin gate accepts `Authorization: Bearer <token>` as well as `X-Admin-Key: <token>`.** The docs, the config reference and `openapi.yaml` all called `admin.token` a bearer token, and every shipped MCP tool (`vibewarden_stream_logs`, `vibewarden_watch_events`, `vibewarden_propose_action`, `vibewarden_list_proposals`) sends it as `Authorization: Bearer` — but `AdminAuthMiddleware` read only `X-Admin-Key`, so all four tools got `401 missing or invalid admin key` against a real sidecar, and the 401 then advertised `WWW-Authenticate: Bearer` for a scheme the gate did not accept. Both header forms are now accepted on `/_vibewarden/admin/*` and the config route; `X-Admin-Key` wins when both carry a value, since it is the VibeWarden-specific header while `Authorization` may be populated for an unrelated reason. The Bearer scheme is matched case-insensitively (RFC 7235) and any other scheme yields no token, failing the same constant-time compare as a wrong one — so this widens the accepted header set, not the accepted secret, and the per-IP lockout (ADR-114) covers both forms identically. The MCP and agent-loop test fakes hand-rolled a `Bearer` string comparison, which is why the suite stayed green while production 401'd; they now sit behind the real `AdminAuthMiddleware`, so this class of drift fails the build. Docs realigned with `openapi.yaml`: both spec copies gain an `AdminBearer` HTTP-bearer security scheme listed as an alternative to `AdminKey`, and `configuration.md` (with a curl example of each form), `production-deployment.md`, `index.md`, `README.md`, `llms-full.txt` and `vibewarden.reference.yaml` now name both headers.

- **fix(#1516): the generated `kratos.yml` pointed every self-service flow at `/auth/*`, a prefix the sidecar has never served.** `internal/config/templates/kratos.yml.tmpl` hard-coded `ui_url: <base>/auth/{login,registration,recovery,settings,verification,error}`, while the built-in auth UI has served `/_vibewarden/{login,registration,recovery,verification,settings}` since v0.20.0 and the auth plugin routes only those five paths. Nothing 404'd: an unmatched path falls through the Caddy route table to the upstream app, so every Kratos-*initiated* redirect (an expired login flow, the logout return URL, the error page) quietly landed on the app's own 404 instead of a login page. Following a link to `/_vibewarden/login` by hand still worked, which is what kept this hidden. The flow URLs are now derived from the config rather than hard-coded: a new `Config.AuthFlowURLs()` emits the built-in `/_vibewarden/` paths in the default `auth.ui.mode: built-in`, and in `custom` mode emits the operator's `auth.ui.*_url` values (absolute URLs as-is, a bare path resolved against `tls.domain` or `localhost:<server.port>`) — previously custom mode was ignored by the template entirely and got the same wrong `/auth/*` URLs as everyone else. Two flows have no page of their own and fall back to `auth.ui.login_url` / the built-in login page: the **error** flow, because the sidecar serves no error page and a recoverable landing beats a fall-through to the app, and, in custom mode only, the **verification** flow, because `auth.ui` has no `verification_url` key. `test/architecture/kratos_ui_urls_test.go` pins the invariant behaviourally — it starts the real auth UI handler and requires a 200 from every path the generator emits, and separately requires the auth plugin to contribute a Caddy route for each, so neither half of the pair can drift again. **Fixing this requires regenerating** (`vibew generate` / `vibew bundle`, then recreate the `kratos` container); a `kratos.yml` pinned through `overrides.kratos_config` is copied verbatim and must be edited by hand. Migration note in `docs/upgrading.md`. The boot-time warning for a `ui_url` the sidecar does not serve is deliberately left as a follow-up.

- **fix(#1526): streaming responses (SSE, chunked LLM token streams) are no longer buffered until the connection closes.** An upstream serving `text/event-stream` streamed correctly when hit directly, but through the sidecar the client got zero bytes until the response ended. Five response-writer wrappers embed `http.ResponseWriter` to capture the status code or byte count (`internal/middleware/{metrics,tracing,access_log}.go`, `internal/adapters/caddy/{circuit_breaker,timeout}_handler.go`) and none re-exported `Flush()` or `Unwrap()`. Embedding promotes `Write` and `WriteHeader` but *hides* the optional interfaces: a handler's `w.(http.Flusher)` assertion failed, and Caddy's reverse proxy, which flushes through `http.ResponseController`, could not reach the real writer because the chain had no `Unwrap() http.ResponseWriter` to walk. Every wrapper now implements both, delegating through the single helper `middleware.FlushResponseWriter`; the three status-capturing wrappers pin their status code on the first flush, since that is the point the response is committed. `Unwrap()` also restores `ResponseController` access to `Hijack` and the deadline setters for anything further down the chain. Two behavioural tests cover it (a method-set assertion would pass while the proxy silently buffered): a real HTTP server and client per wrapper and per full chain, where the handler flushes one chunk and then blocks — the test fails if the bytes do not arrive while the handler is still open — plus the same for the two Caddy modules. `test/architecture/response_writer_flush_test.go` pins the invariant by AST-scanning `internal/` for structs that embed `http.ResponseWriter` without declaring `Flush`/`Unwrap`, so the next wrapper cannot reintroduce the bug. Three settings still buffer or cut a stream **by design** and are now documented in `docs/websockets.md` (renamed in the nav to "WebSockets and streaming"): `resilience.timeout` (default `30s`) counts the whole response as one request and cuts a longer-lived stream; `resilience.retry.enabled` buffers the full response in memory so a failed attempt can be discarded, so it must stay off on streaming routes; and egress `response_validation.enabled` buffers the body to validate it against its schema.

- **fix(#1527): the built-in login, registration and settings pages now recover from an expired flow instead of demanding a refresh.** A login page left open past `selfservice.flows.login.lifespan` (kiosks, shared devices, a tab from this morning) failed every submit with "Login session expired. Please refresh." and Kratos logged *the anti-CSRF cookie was found but the CSRF token was not included*. Three bugs compounded: the submit handler re-fetched the flow without checking the status, so a `410 Gone` body was treated as a flow; it always posted the `csrf_token` captured at page load even though Kratos rotates the anti-CSRF cookie whenever it hands out a new flow; and it fed error envelopes (which have no `ui` section) into `applyFlow`, which blanked the hidden flow id and stranded every later click on the "please refresh" branch. Each page now has `startFreshFlow()`/`currentFlow()`: a dead flow is re-initialised in place via `/self-service/<flow>/browser`, the `csrf_token` is read from the flow that is about to be posted rather than from page load, a `410` or `security_csrf_violation` on the POST itself is retried exactly once against a fresh flow, and `applyFlow` ignores any body without `ui`. The page is never reloaded, so the typed identifier and password survive the recovery. Social buttons are now replaced rather than appended on re-render, which also fixes providers stacking up after a validation error. Re-initialisation content-negotiates JSON (`Accept: application/json`) and reads the flow straight out of the response body, falling back to the final URL of a followed redirect. It deliberately does **not** read the `Location` header of the `303`: with `redirect: 'manual'` a browser hands back an opaque-redirect response whose status is `0` and whose header list is empty, so that header is always `null` and the recovery would dead-end on "Could not start login session". Covered by a new behavioural suite (`make check-authui-js`) that runs the real inline page script in a Node vm against a fake Kratos modelling flow expiry, CSRF rotation and the browser's opaque-redirect filtering; it fails against the pre-fix templates.

- **fix(#1504): agents no longer post another agent's verdict, because comment bodies are now composed inline instead of read from a fixed scratchpad file.** On 2026-09-04 the reviewer agent posted "CHANGES REQUESTED" comments on PRs #1503 and #1498 whose bodies described entirely different PRs, and an architect comment on issue #1495 was corrupted the same way; the merge gate caught it only because the comment text did not match the PR. Cause: agents wrote the verdict with `cat > <scratchpad>/review.md` and posted it with `--body-file`, but the agent shell runs zsh with `noclobber`, so the redirect onto a file an earlier subagent had already created fails with `file exists:`, the heredoc write is skipped, the command list keeps running, and `--body-file` posts whatever the previous agent left behind. The scratchpad is shared by every subagent in a session, so the stale content is always someone else's review. The agents' structured return values were correct throughout — only the posted comments were wrong. `CLAUDE.md` and all eight agent definitions (`reviewer`, `writer`, `architect`, `pm`, `dev`, `auditor`, `pentester`, `release`) now require the inline form `--body "$(cat <<'EOF' ... EOF)"` and ban `--body-file` (and `--notes-file`) from a fixed path (`mktemp` plus `>|` plus a read-back check if a file is truly unavoidable); the two `--body-file /tmp/finding.md` snippets in `auditor.md` and `pentester.md` are rewritten, as is `release.md`, which wrote public release notes to the fixed path `/tmp/release-notes.md` and published them with `gh release create --notes-file`. `/issue-pipeline` gains a defence in depth: after each review round it reads what is actually posted on the PR (issue comments and review bodies) as one timestamp-tab-first-line record per comment (`--jq '.[] | "\(.created_at)\t\(.body | split("\n")[0])"'` for issue comments, `.submitted_at` for reviews, which have no `created_at`, so multi-paragraph bodies cannot run together and "newest" is decidable on both surfaces) and compares those first-line verdicts against the agents' structured verdicts, escalating with the mismatch spelled out rather than dispatching a fix round against phantom blocking items. `writer.md` also switches from `gh pr review --approve` to a `Writer Agent: APPROVED` PR comment, which is what the merge gate and the new cross-check have always looked for. Agent tooling only — no change to any shipped binary, config, or runtime behaviour.

- **fix(#1311): the connection-limit listener now records a refusal before it closes the connection (carried by #1495).** `connLimitedListener.Accept` closed the over-cap connection and only then called `noteRefusal()`, so the FIN/RST reached the client while `refused` was still at its old value and the throttled `connection refused: max_connections reached` warning had not been emitted. That is an observability gap on the real path — anything correlating a client-side reset with the sidecar's counter or log (an operator, a scrape, the behavioural test) could see the refusal on the wire but not in the numbers. The two statements are swapped so the counter and the log strictly precede the close; refusal logging is throttled to once per 30s, so this does not put log I/O on the flood path. `TestConnLimitListener_RefusesOverCap` asserts the count without polling again, which is what makes a future reordering fail: under `GOMAXPROCS=2` with 20 spinning processes it failed 9 times in 120 runs before the fix and 0 times in 120 after.

- **fix(#1305): `vibew upgrade` now actually retries with sudo when the install directory is not writable.** `installBinary` classified the failure from `atomicReplace` with `os.IsPermission`, but `atomicReplace` wraps every error it returns with `fmt.Errorf(...%w)` and `os.IsPermission` does not unwrap — so the condition was never true and the documented sudo retry was dead code. Users who installed to `/usr/local/bin` without write permission got a bare "permission denied" instead of the retry the godoc promises. Classification moves to `errors.Is(err, fs.ErrPermission)`, which unwraps. Found while backfilling coverage for the same function; the retry is now covered by a test that puts a stub `sudo` on `PATH` rather than escalating.

- **fix(#1402): per-user rate-limit audit events now record which user tripped the limit.** `audit.rate_limit.hit` was constructed with `audit.Actor{IP: clientIP}` on both branches, so a per-user 429 produced an audit entry with an empty `Actor.UserID` — you could see that a user limit fired but not whose. The user branch now threads the already-resolved user ID (context identity, falling back to `X-User-Id`) into `Actor.UserID`; the per-IP branch keeps an empty user ID, since the request has not been attributed to a user at that point. Pre-existing since the audit event was introduced, not a regression from #1279. Guarded by a table-driven test asserting the actor fields on all three paths.

- **fix(#1476): unbreak `mkdocs build --strict`, which had failed on every push to main since April and froze the published site at vibewarden.dev/docs for 4.5 months.** The `Deploy docs to vibewarden.dev` workflow aborted on 12 strict-mode warnings, all of them relative links from `docs/` to repository files that are not part of the MkDocs site and therefore cannot resolve: `../CLAUDE.md`, `../README.md#bundle`, `../CHANGELOG.md` and four `../decisions/adr-0NN-*.md` targets across `docs/architecture.md`, `docs/deploy-reference.md`, `docs/multi-app.md` and `docs/guide/bundle-to-vps.md`. All are now absolute `https://github.com/vibewarden/vibewarden/blob/main/<path>` URLs. Two of those targets were wrong independently of how they were written, and are corrected rather than merely rewritten: the ADR-085 link named a filename that never existed (`adr-085-vibew-bundle-cli.md`, actually `adr-085-vibew-bundle-compose-only.md`), and the two `README.md#bundle` links in `docs/deploy-reference.md` named an anchor README has never had, so both landed at the top of the file; they now point at `#cli-reference`, the section holding the `vibew bundle` row, and the sentence promising a flag reference points at `docs/guide/bundle-to-vps.md`, which is where the flag table actually lives. `docs/secret-management.md` linked to `../troubleshooting.md` where the page is a sibling, and the `docs/troubleshooting.md` exit-code table linked to an anchor with a doubled hyphen (`...-vibew-dev--bundle--logs`) that does not match the slug Python-Markdown generates. **Prevention:** because the deploy workflow only runs on push to main and is not a required check, nothing surfaced the breakage for 40 consecutive runs. CI gains a `docs-build` job that runs the same `mkdocs build --strict` on every pull request touching `docs/**`, `mkdocs.yml`, or either workflow file, via a new `docs` output on the existing path-filter job (kept separate from the `code` filter so docs-only PRs still skip the Go suite). No content changes beyond link targets and the one sentence reworded to match them.

- **docs(#1310): remove the last `vibew deploy` zombie references left by the ADR-086 sunset.** `config.LoadStrict`'s godoc still named `vibew deploy` as one of its two callers; the real callers are `vibew validate` and `vibew bundle` (`vibew doctor` does not call it). The canonical diagnostic fixture in `internal/domain/health/diagnostic_test.go` seeded the detail string `Check logs with: vibew deploy logs`, which reads as a real command to anyone (human or agent) using that test as the reference diagnostic shape; it now says `vibew logs`, the shipped wrapper. The "deploy was removed" paragraph in the scaffolded `AGENTS-VIBEWARDEN.md` (and its mirrors in `docs/examples/AGENTS-VIBEWARDEN.md`, `llms-full.txt`, and `docs/deploy-reference.md`) described the fallout as "cobra's `unknown command \"deploy\"` error"; all four now quote the behaviour verified against the built binary — exit 1 with `Error: unknown command "deploy" for "vibew"` on stderr. `TestRootCmd_Deploy` gains `deploy logs` and `deploy logs --lines 50` cases so the removal of the subcommand is asserted, not just the verb. No runtime behaviour changes.

- **fix(#1458): two local-host hardening fixes from the 2026-05-03 security audit (consolidates #1283, #1285).** (1) **Bundle directories are now owner-only.** Every directory `vibew bundle` creates was mode `0750`, so any other local OS user in the owner's group could list and traverse a tree that holds `.env` and `.credentials`. All four `MkdirAll` sites (`internal/cli/cmd/bundle.go`, the `writeFile` helper in `internal/app/bundle/bundle.go`, and both `.vibewarden/` creations in `internal/app/bundle/input_digest.go`) now share a single `bundle.DirPerm = 0700` constant. They had to change together: the default output directory is `.vibewarden/bundle`, so two of the sites create `.vibewarden` and `MkdirAll` is a no-op on an existing directory, which would otherwise make the on-disk mode depend on which call ran first. File modes are untouched (`.env` and `.credentials` were already `0600`). There is no retroactive `chmod`: directories created by an earlier version keep their existing mode, so tighten them by hand or delete and re-bundle. (2) **Docker daemon stderr is sanitized before it reaches the terminal.** `renderDockerUnavailable`, shared by `vibew bundle`, `vibew dev`, and `vibew logs`, echoed the captured stderr of a failed `docker` invocation verbatim, so a malicious or corrupted daemon could inject ANSI escape sequences to clear the screen, recolour text, rewrite the window title, or hide output with a carriage return. Stderr now passes through a rune state machine that drops ESC-introduced sequences (CSI, OSC/DCS/SOS/PM/APC, and short two-byte forms), C0 controls, DEL, and the C1 range `0x80` to `0x9F` (which includes the single-byte CSI and OSC forms), while preserving newline, tab, and all printable text including non-ASCII. A newline also terminates any in-progress sequence, so an unterminated OSC cannot swallow the rest of the message; escape-only stderr collapses to empty and the "Underlying error:" section is omitted rather than printed blank. Output for ordinary printable stderr is byte-identical to before.

- **fix(#1457): two `vibew dev` robustness fixes (consolidates #1308, #1309).** (1) The project walk shared by the staleness check and the input digest now hard-ignores the root-relative path `.claude/worktrees`. On this repository that subtree holds one checkout per in-flight agent task (45k files) and cost ~800ms on every `vibew bundle`; skipping it brings the walk to ~50ms. The match is on the root-relative path, not the bare directory name, so the rest of `.claude` (agent definitions, settings) still counts toward freshness and a project's own top-level `worktrees/` directory is unaffected. (2) `docker image inspect` is now bounded by a 10s deadline when the caller supplied none, so an unresponsive Docker daemon (common while Docker Desktop restarts) can no longer hang `vibew dev` startup indefinitely — a healthy daemon answers in under 200ms. A blown deadline is reported as `ports.ErrDockerDaemonNotRunning`, so the callers that already degrade gracefully on `ports.ErrDockerUnavailable` (preflight WARN, the `vibew dev` image-identity check) treat a frozen daemon exactly like an absent one instead of failing hard. Caller-supplied deadlines are honoured unchanged. No config or output-format changes.

- **docs(#1456): correct three stale documentation details (consolidates #1282, #1287, #1288).** `decisions/README.md`: the 2026-05-03 audit summary said "42 KEEP"; the index table supports 45 (69 ADRs audited − 13 demoted − 11 tombstoned). The summary now states the audited ADR range so the arithmetic stays verifiable as new ADRs are appended, and notes that supersession notes on demoted or tombstoned rows (such as ADR-079) are not part of the 3 KEEP status banners. The v0.18.7 entry below repeats the wrong "42 KEEP" and gains a correction annotation rather than being rewritten, since released entries are historical records. `decisions/adr-075-*.md`: gains the Historical banner and lineage note the audit called for but never applied: `vibew deploy` was sunset by ADR-086, and the "local resolution, no remote patching" principle now lives in `vibew bundle` (ADR-085). CHANGELOG v0.18.0: the "Migrating from v0.17.0" table and the `deploy.sh` removal note still prescribed `scp -r .vibewarden/bundle/*`, the dotfile-dropping form replaced by the tar pipe in v0.18.4 (#1217); both now show the tar pipe with an annotation, and the placeholders are bracketed per the cross-LLM literal-vs-template rule. No code changes.

- **fix(#1454): repair the `internal/adapters/metrics` integration test build.** `otel_integration_test.go` still called `provider.Init(ctx, name, version)` after the fourth `ports.TelemetryConfig` parameter was added, so `go vet -tags integration ./...` failed to compile repo-wide and the integration suite could not run. The call now passes a `TelemetryConfig` with the Prometheus exporter enabled, matching what the test asserts.

- **fix(#1535): `vibew down` is a no-op again when nothing was generated, and the integration suite is green.** Three problems, one red `make integration`. (1) `vibew down` in a directory with no `.vibewarden/generated/docker-compose.yml` exited 1 with `docker compose down: exit status 1`: the no-op summary was only reachable *after* compose had been invoked with a `-f` path that does not exist. The service now stats the compose file first and prints `No running services. Nothing to do.` before touching Docker (and before prompting for `--volumes`, so it never offers to delete data that cannot exist). (2) `vibew bundle` rendered `image:` with no value for the sidecar service, making the bundled `docker-compose.yml` invalid (`services.vibewarden.image must be a string`). `SidecarImage`/`SidecarPullPolicy` are computed from the CLI build version (ADR-106) rather than read from YAML, so the production merge that reloads the config from disk wiped them. They are recomputed after the merge. (3) The two `obs_lifecycle` integration tests called `vibew init --upstream-port`, a flag that has never existed, and swallowed the resulting error in a `t.Skipf`, so they had never once run; they now use `--port`, fail loudly on an init error, and are backed by a shared busybox fixture image that satisfies the generated `app` healthcheck (the same fixture gives `TestBundle_CLI_UpAndHealthy` the `foo-app:latest` image it always assumed but never built). The obs test also publishes the sidecar on a random high port instead of 8443 so an unrelated local stack cannot fail it. `make integration` now runs with a 900s timeout, since the obs lifecycle test really does start the whole observability profile.
...
- **docs(#1284): mark `docs/internal/scaffold-bootstrap.md` as a historical record instead of a specification (audit `audit:2026-05-03`).** The file's directory tree was introduced by "The dev agent must create exactly this structure", but the tree is the 2026-03-20 bootstrap layout and has no `internal/cli/`, `internal/mcp/`, `internal/middleware/`, `internal/quality/`, `internal/apispec/` or `internal/schema/`, no expanded `internal/adapters/` subtree, and none of `decisions/`, `docs/`, `examples/`, `npm/`, `schema/`, `scripts/`, `site/` or `test/`. Read as prescriptive (which `docs/internal/` invites, and which an agent following the sentence literally would do), it teaches a repository layout that has been wrong for months. The heading now carries a "historical snapshot, not a specification" note, and the tree is introduced as what the bootstrap issue specified. The note sends the reader to the tree itself (`find internal -maxdepth 1 -type d`, `ls -d */`) rather than to another document: **CLAUDE.md § Directory layout** is the architectural contract, but its tree is abbreviated and has the same gaps, so labelling it "the authoritative current layout" would have reproduced the staleness this entry fixes. The same note is applied to the `## Specification` section above it, whose pinned Go 1.26.1 is stale for the same reason. Docs only.
- **docs(#1461): record as ADR-108 the decision not to embed an authorization policy engine (Casbin or similar) in the sidecar.** The sidecar's authorization ceiling is *path × method × identity attribute*, and it is already fully covered: `auth.role_paths` (Kratos role vs. path, `internal/adapters/caddy/auth_handler.go`), `auth.api_key.scope_rules` (`auth.ScopeRule` in `internal/domain/auth/scope_rule.go`), and the `X-Admin-Key` gate on `/_vibewarden/admin/` (`internal/middleware/admin_auth.go`). A policy engine adds no capability at that ceiling while costing a second configuration language next to the flat-YAML `vibewarden.yaml` the strict loader validates, a runtime-evaluated matcher whose miss mode is fail-open (the failure already shipped twice, in #1302 and #1176/#1177), and more edges on a dependency graph that is already oversized (#1299). Anything needing resource state — ownership, tenancy, per-record visibility — is the application's job, not the proxy's. The ADR records an evolution path if the need ever appears: unify `role_paths` and `scope_rules` into one rules block with *opt-in* deny-by-default (`ScopeRule` is open-by-default today, so flipping it silently would lock out live deployments), then CEL for ABAC conditions via the already-present `google/cel-go`, and Ory Keto — not Casbin — as a separate service if relation-based authz gets real demand. Documented in `decisions/adr-108-no-authz-policy-engine-in-the-sidecar.md`, with `decisions/README.md` gaining the index row and its ADR-108 reservation note discharged. No code changes.
- **docs(#1347): commit the two outstanding dependency-license waivers as ADR-109 (consolidates #1292, #1293).** `CLAUDE.md`'s approved-license list (Apache 2.0, MIT, BSD-2, BSD-3) is an allow-list, and the 2026-05-03 audit found two transitive dependencies outside it: `github.com/go-sql-driver/mysql` (MPL-2.0, reached through `caddy/modules/caddypki/acmeserver` → `smallstep/nosql`) and `github.com/zeebo/blake3` (CC0-1.0, reached through `caddy/v2` → `certmagic`). Both waivers were decided when #1292 and #1293 were closed, but the MPL ADR was only ever written to a working tree — the number it claimed, ADR-104, was later reused for the OpenBao prod-init decision, so #1292's closing comment cites an unrelated ADR and neither waiver exists in git. Both are now recorded in `decisions/adr-109-license-waivers-mpl-2-mysql-driver-cc0-blake3.md`: the mysql waiver is conditional on five conditions (indirect-only, unmodified, binary-only distribution, dormant code path, upstream remediation tracked) and lapses if any stops holding; the blake3 waiver is unconditional, since CC0 imposes no obligations. Condition 5 states the current fact — no upstream issue exists yet — and names #1467 as the owner of filing it, so a license sweep can tell a lapsed waiver from an unsatisfied condition. Waivers are per-dependency, not per-license. `decisions/README.md` gains the index row plus notes recording the ADR-104 double-assignment and the ADR-108 reservation. The approved-license list is unchanged and no code changes: both dependencies stay `// indirect` in `go.mod`.
- **docs(#1517): document the `kratos`-mode identity-header contract where auth is documented, and delete the phantom `X-Session-ID`.** `docs/identity-providers.md` documented `claims_to_headers` for `jwt` mode but said nothing about what `kratos` mode injects, so the only full description of the Kratos header set lived in one framework example. It now has an "Identity headers in `kratos` mode" section covering all four headers and their Kratos sources (`X-User-Id` ← `identity.id`, `X-User-Email` ← the `email` verifiable address, falling back to `identity.traits.email`, `X-User-Verified` ← the `email` verifiable address, `X-User-Role` ← `identity.traits.role`), that the set is **fixed** with no `claims_to_headers` equivalent, which headers can be omitted (`X-User-Id` when empty, `X-User-Email` when the identity has neither an `email` verifiable address nor a string `email` trait) versus always set (`X-User-Verified`, `X-User-Role`), the incoming-`X-User-*` strip, and the one behaviour app code most often gets wrong: on `auth.public_paths` the headers are **absent** rather than the request being redirected, so a missing `X-User-Id` there means anonymous, not error. The same block is added to `llms-full.txt`, and the AI-agent-readable summary JSON gains `kratos_headers`, `kratos_headers_configurable: false` and `session_header_exists: false`. **`X-Session-ID` never existed** — nothing in `internal/adapters/caddy/auth_handler.go` sets it, in any mode — but `docs/examples/{express,nextjs,django,rails}.md` all listed it, and the Express, Django and Rails middleware samples read it, so an app following them wired up a permanently empty session ID. The header is removed from all four; where a sample used it (Express `req.user`/pino `customProps`, Django `VibeWardenUser`/`VibeWardenIdentity`, Rails Rack env/`log_tags`) it is replaced by `X-User-Role`, which is real and was missing from the Django and Rails header tables entirely. Two drive-by fixes in the same files: the Express TypeScript `VibeWardenUser` interface declared `sessionId` but not `role`, while the function assigned `role`, so the snippet did not compile; and the tables spelled the header `X-User-ID` against the wire's `X-User-Id`. Each example now cross-links the identity-providers section. Docs only, no code changes.
...
- **fix(#1439): sync `llms-full.txt` with `vibewarden.reference.yaml` and the config structs — six audited drift findings in one pass.** `llms-full.txt` is the file AI agents load as their primary config reference, so every gap between it and the code produces a config that either omits valid options or fails `vibew validate`.
  - **#1327 — `webhooks.signature_verification.paths` field names.** The entry schema documented `secret: ${STRIPE_WEBHOOK_SECRET}`, but `WebhookSignaturePathConfig` (`internal/config/webhooks.go`) declares `secret_env_var` and takes a **bare** env-var name. It also invented two fields that exist nowhere in the code — `algorithm` and `tolerance` — which the strict loader rejects as unknown keys. Corrected the table and both examples, added a `provider: generic` example (the only provider for which `header` is meaningful), and stated explicitly that no algorithm/tolerance knob exists (generic is HMAC-SHA256 hex over the raw body).
  - **#1326 — seven sections present in `vibewarden.reference.yaml` but absent from `llms-full.txt`.** Added `compression`, `response_headers`, `input_validation`, `error_pages`, `maintenance`, `tls.cert_monitoring`, and `log.access_log`, each with field/type/default tables verified against the Go structs and `setDefaults()` rather than against other docs. The `tls` table also gained `email`, `acme_ca`, and `skip_rate_limit_check`, and the provider list now names all six accepted values.
  - **#1280 — `telemetry.traces.enabled`.** Verified rather than changed: the key is real (`TracesConfig` in `internal/config/telemetry.go`, wired in `internal/adapters/otel/provider.go`), so `llms-full.txt` was right and the reference YAML was the side with the gap — already closed by #1373. Both files now agree with the code; no edit required.
  - **#1278 — `waf.acknowledge_log_mode`.** Added to the WAF reference block, together with the exact condition under which the `vibew validate` check fires (`waf.mode` is literally `"log"` **and** a `vibewarden.production.yaml` exists) and the fact that `log` is an accepted alias for `detect`.
  - **#1277 — `GET /_vibewarden/me` missing from the generated `AGENTS-VIBEWARDEN.md`.** The paragraph was documented in `docs/examples/AGENTS-VIBEWARDEN.md` but not in `internal/cli/templates/agents/agents-vibewarden.md.tmpl`, so the file `vibew init` actually writes never mentioned the endpoint — frontend agents fell back to reading `X-User-*` headers, which is awkward from a browser. Added to the template.
  - **#1275 — Step 9 rsync guidance.** The VPS walkthrough used a tar pipe in Step 7 and rsync in Step 9 with no explanation, leaving agents unsure which is authoritative. Step 9 now states the actual invariant (both are dotfile-safe, which is the hard requirement because the bundle contains `.env` and `scp -r bundle/*` silently drops it), notes rsync's delta transfer as the reason it is preferred for redeploys, and gives the tar pipe as the fallback for hosts without rsync. The rsync arguments are now single-quoted to match the surrounding `ssh` lines.

  Two further defects were found while verifying the above against the code, and are fixed here because both mislead readers of the files this issue is about:
  - **`vibewarden.reference.yaml` documented a key that does not exist.** `social_providers` was nested under `kratos:`, but the field lives on `AuthConfig` as `auth.social_providers`. The reference file — advertised as documenting "EVERY supported option" — therefore did not itself pass `LoadStrict`: `vibew validate` rejects `kratos.social_providers` as an unknown key, so anyone copying that block got a config they could not bundle. The block moved to `auth:` and was rewritten from a list of bare provider names to the object form the struct actually requires (`provider` + `client_id` + `client_secret`, plus `team_id`/`key_id` for Apple and `id`/`issuer_url` for generic OIDC). `docs/social-login.md` carried the same wrong block and the same incorrect claim that credentials live only in `kratos.yml`; both are corrected — `vibew generate` renders these entries into the generated `kratos.yml` and writes the matching claim mappers.
  - **`llms-full.txt` contradicted itself on social login**, documenting the `auth.social_providers` fields and then stating that social login is configured in `kratos.yml` and "not vibewarden.yaml". Replaced with the actual relationship between the two files.

  Three regression guards were added so this class of drift fails the build instead of reaching users: `TestReferenceYAML_PassesStrictLoader` runs `vibewarden.reference.yaml` through the same `LoadStrict` path as `vibew validate` (the pre-existing test used `LoadRaw`, which tolerates unknown keys and is why the bad key survived); `TestLLMSFullTxt_CoversEveryReferenceSection` asserts every top-level and second-level key in the reference YAML is discoverable in `llms-full.txt` (one-directional, with a documented skip for the deprecated `metrics:` section); and `TestAgentsVibewardenTemplate_SidecarHeadersMatchCanonicalExample` pins the rendered template's "Sidecar-injected headers" section byte-for-byte against `docs/examples/AGENTS-VIBEWARDEN.md`.

- **fix(#1339): stop the sidecar spamming `failed to upload metrics` on every OTLP export interval.** In dev mode the `observability` compose profile is off, so `otel-collector` is not running and every OTLP push failed with `dial tcp: lookup otel-collector ... no such host`. The OTel SDK routes background export failures to `otel.Handle`, whose default handler writes to the stdlib `log` package — which Caddy redirects into its own logger at **info** level, producing one noise line per interval (~every 30s) and burying real errors in `vibew logs vibewarden`. The metrics plugin now installs a slog-backed global OTel error handler (`internal/adapters/otel/errorhandler.go`) before any exporter starts: the first occurrence of each distinct error message is logged once at `warn` — so a genuinely misconfigured collector is still visible — and every later occurrence of that message is demoted to `debug` with a `repeat_count` attribute. Dedup state is kept per message in a bounded table, not just for the previous message, because the generated `docker-compose.yml` enables the metric and log OTLP exporters together: their two export loops fail on different intervals and interleave, so single-slot dedup would treat every alternation as new and warn forever. This covers metric, trace and log export failures alike, since all of them funnel through the same global handler.
...
| `cd .vibewarden/bundle && ./deploy.sh user@host` | `ssh '<your-ssh-user>@<your-ssh-host>' mkdir -p /opt/<your-app>` → `tar -czf - -C .vibewarden/bundle . \| ssh '<your-ssh-user>@<your-ssh-host>' 'tar -xzf - -C /opt/<your-app>/'` → `ssh '<your-ssh-user>@<your-ssh-host>' 'cd /opt/<your-app> && docker load -i image.tar && docker compose up -d'` → verify with `curl https://<your-domain>/_vibewarden/health` (port **443** in production, not 8443). The bundle's `README.md` documents this contract. *(Transfer step updated in v0.18.4, #1217. v0.18.0 originally documented `scp -r .vibewarden/bundle/*`, which silently drops dotfiles such as `.env`; use the tar pipe.)* |
...
- **`deploy.sh` no longer ships in the bundle** (#1138). The qr-dali deploy retrospective surfaced that the bundled script's hardcoded healthcheck port (8443, the dev port) reported false failures on successful production deploys (which listen on 443 with TLS). The bundle's `README.md` now describes the deploy contract directly — what the bundle is, where it goes, and the two non-obvious traps (the remote directory must exist before copying; healthcheck port in production is 443, not 8443). Operators and AI agents own the `scp` / `ssh` / `docker compose up -d` chain themselves. ADR-088 marked as superseded. See `CLAUDE.md` §Architecture principles → Artifact policy for the rationale (no example-shaped middle-ground artifacts).
  ...
  **Migration:** if you scripted around `cd .vibewarden/bundle && ./deploy.sh user@host`, replace it with the steps the bundle README now describes: `ssh '<your-ssh-user>@<your-ssh-host>' mkdir -p /opt/<your-app>` → `tar -czf - -C .vibewarden/bundle . | ssh '<your-ssh-user>@<your-ssh-host>' 'tar -xzf - -C /opt/<your-app>/'` → `ssh '<your-ssh-user>@<your-ssh-host>' 'cd /opt/<your-app> && docker load -i image.tar && docker compose up -d'` → `curl https://<your-domain>/_vibewarden/health`. *(Transfer step updated in v0.18.4, #1217. The `scp -r .vibewarden/bundle/*` form documented here at v0.18.0 silently drops dotfiles such as `.env`.)*
