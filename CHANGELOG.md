...
- **test(#1307): `BenchmarkProxy_WithWAF_WithBody` — the WAF benchmark now measures the body-scan path it was supposed to measure all along (audit `audit:2026-05-03`).** `BenchmarkProxy_WithWAF` builds its request with `newBenchRequest()`, a `GET` with no body, so the published WAF overhead only ever covered the query-parameter and header scan. The body branch allocated its 8 KB read buffer and then found zero bytes to match rules against, which is why the number looked reassuringly small while being unrepresentative of any JSON API. The new benchmark sends a `POST` carrying the same small JSON payload as the domain-level `BenchmarkScanRequest_Typical`, so the two are directly comparable. On an M1 Max the gap is **~1.4 µs vs ~9.0 µs of WAF overhead per request, 6.5x** — real, and worth budgeting for at 1 000 RPS (~9 ms of CPU per wall-clock second). A companion `BenchmarkProxy_DirectPassthrough_WithBody` is added as the baseline: a `POST` with a body costs more to construct than a bodyless `GET`, and charging that to the WAF would overstate the overhead. `docs/performance.md` gains a second results table for the body-bearing shape, re-measured on current hardware and Go 1.27 (the previous numbers were from a Rosetta VM), and the WAF observation now says which number to budget with. Benchmarks only — no production code changed.
