...
- **fix(#1476): unbreak `mkdocs build --strict`, which had failed on every push to main since April and froze the published site at vibewarden.dev/docs for 4.5 months.** The `Deploy docs to vibewarden.dev` workflow aborted on 12 strict-mode warnings, all of them relative links from `docs/` to repository files that are not part of the MkDocs site and therefore cannot resolve: `../CLAUDE.md`, `../README.md#bundle`, `../CHANGELOG.md` and four `../decisions/adr-0NN-*.md` targets across `docs/architecture.md`, `docs/deploy-reference.md`, `docs/multi-app.md` and `docs/guide/bundle-to-vps.md`. All are now absolute `https://github.com/vibewarden/vibewarden/blob/main/<path>` URLs; the ADR-085 link was additionally pointing at a filename that never existed (`adr-085-vibew-bundle-cli.md`, actually `adr-085-vibew-bundle-compose-only.md`). `docs/secret-management.md` linked to `../troubleshooting.md` where the page is a sibling, and the `docs/troubleshooting.md` exit-code table linked to an anchor with a doubled hyphen (`...-vibew-dev--bundle--logs`) that does not match the slug Python-Markdown generates. **Prevention:** because the deploy workflow only runs on push to main and is not a required check, nothing surfaced the breakage for 40 consecutive runs. CI gains a `docs-build` job that runs the same `mkdocs build --strict` on every pull request touching `docs/**`, `mkdocs.yml`, or either workflow file, via a new `docs` output on the existing path-filter job (kept separate from the `code` filter so docs-only PRs still skip the Go suite). No content changes beyond link targets.
