...
- **fix(#1536): `vibew add auth` wrote Kratos URLs the sidecar container cannot reach, so the documented auth path was broken on first run.** The block it appended pointed `kratos.public_url` / `kratos.admin_url` at `http://localhost:4433` / `:4434`, but the sidecar dials those from inside its own container, where `localhost` is the sidecar and not the `kratos` service: the admin API answered `503 service_unavailable` (`dial tcp [::1]:4434: connect: connection refused`), `/self-service/login/browser` was a 502, and unauthenticated requests were redirected off the sidecar to `http://localhost:4433/...` because the same block wrote an absolute `auth.login_url` to a port the generated stack never publishes. `vibew add auth` now writes `http://kratos:4433` / `http://kratos:4434` — matching `examples/demo-app/vibewarden.yaml`, and already understood by `vibew status`, which rewrites that host to a published port when probing from the host — and omits `auth.login_url` entirely, so the middleware's sidecar-relative `/self-service/login/browser` default applies and the login flow stays on the sidecar. **Existing projects are not migrated**: a `vibewarden.yaml` already generated by `vibew add auth` keeps the broken values, so fix it by hand (set both URLs to the `kratos` host and delete `auth.login_url`) or delete the two blocks and re-run the command. Run the binary outside Docker and the `127.0.0.1` defaults still apply; set the URLs to whatever resolves for you. The false claim in `vibewarden.reference.yaml` that the generated compose injects `VIBEWARDEN_KRATOS_PUBLIC_URL` / `VIBEWARDEN_KRATOS_ADMIN_URL` is corrected — no such variable is emitted, so the config file is the only source of these URLs (setting them yourself in a hand-written compose file still overrides, like any key). `TestGenerate_Integration_AddAuthKratosURLsAreComposeServiceNames` pins the invariant end to end: it runs the real toggler, loads the resulting config, generates the stack, and requires each Kratos URL host to be a service defined in that same `docker-compose.yml` — an assertion on the literal string would have been just as green before the fix.
