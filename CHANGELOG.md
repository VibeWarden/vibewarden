...
- **fix(#805): bump `testcontainers-go` to v0.43.0 — drops vulnerable `github.com/docker/docker` (CVE-2026-34040, CVE-2026-33997).** v0.43.0 migrated off `github.com/docker/docker` onto the new modular `github.com/moby/moby/api` + `github.com/moby/moby/client` packages. The explicit pin of `github.com/docker/docker v28.5.2+incompatible` — which carried a high-severity (CVE-2026-34040) and a medium-severity (CVE-2026-33997) vulnerability — has been removed from `go.mod`. Production risk was always zero (no Docker daemon at runtime); the vulnerable module was test-only. `github.com/moby/moby` is Apache-2.0 licensed.
