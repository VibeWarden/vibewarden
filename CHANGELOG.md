...
- **fix(#1369): OpenBao container and seed-secrets job are no longer generated when `secrets.store` is `builtin` (or unset).** The compose template previously guarded all OpenBao infrastructure on `.Secrets.Enabled`, so users who set `secrets.enabled: true` with the default `store: builtin` (a self-contained encrypted file store) got an unwanted OpenBao container. The guard is now `UsesOpenBao` (enabled AND store == "openbao"). The `EgressNoProxy` helper is fixed to match. Existing `store: openbao` users are unaffected.
